...
// src/admin/services/showtimes.js
...
const enrichShowtimesWithNames = async (showtimes) => {
  try {
    ...
    return showtimes.map((st) => {
      // Dùng String() để đảm bảo so sánh đúng bất kể là "1" hay 1
      const movie = movies.find(
        (m) => String(m.id) === String(st.movieId) || m.movie_id === st.movie_id
      );
      const cinema = cinemas.find(
        (c) =>
          String(c.id) === String(st.cinemaId) || c.cinema_id === st.cinema_id
      );
      const room = rooms.find(
        (r) => String(r.id) === String(st.roomId) || r.room_id === st.room_id
      );
      ...
      return {
        ...
        roomName: room?.name || room?.room_name || "N/A",
      };
    });
  } catch (error) {
