...
// Interceptor: Xử lý lỗi Token hết hạn (401)
...
export const login = async (username, password) => {
  try {
    // Gọi API login thật của Backend
    ...
    // Phân quyền: Nếu có employee thì lấy role của employee, ngược lại là member
    const role = decoded.employee?.role || "member";

    const userData = {
      id: decoded.sub, // Thường là ID người dùng
      username: decoded.username,
      role: role,
      info: decoded.employee || decoded.member || {},
    };

    localStorage.setItem("token", token);
    localStorage.setItem("user", JSON.stringify(userData));

    return { result: { token, user: userData } };
  } catch (error) {
...
export const getBookedSeatsByShowtime = async (showtimeId) => {
  const response = await fetch(`${BASE_URL}/bookings?showtimeId=${showtimeId}`);
  const bookings = await response.json();
  // Gom tất cả các ghế đã có trong bookings thành 1 mảng duy nhất
  return bookings.reduce((acc, b) => [...acc, ...b.seats], []);
};
...
export const createBooking = async (bookingData) => {
  try {
    ...
    const newBooking = {
      ...
      updatedAt: new Date().toISOString(),
      booking_details: bookingData.seats.map((seatId) => ({
